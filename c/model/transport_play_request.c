...
#include <string.h>
#include <stdio.h>
#include "transport_play_request.h"
...
transport_play_request_t *transport_play_request_create(
    ...
    transport_play_request_t *transport_play_request_local_var = malloc(sizeof(transport_play_request_t));
    if (!transport_play_request_local_var) {
        return NULL;
    }
    transport_play_request_local_var->transports = transports;

    return transport_play_request_local_var;
}


void transport_play_request_free(transport_play_request_t *transport_play_request) {
    if(NULL == transport_play_request){
        return ;
    }
    listEntry_t *listEntry;
    if (transport_play_request->transports) {
        list_ForEach(listEntry, transport_play_request->transports) {
            d3api_locator_free(listEntry->data);
        }
        list_free(transport_play_request->transports);
        transport_play_request->transports = NULL;
    }
    free(transport_play_request);
}

cJSON *transport_play_request_convertToJSON(transport_play_request_t *transport_play_request) {
    cJSON *item = cJSON_CreateObject();

    // transport_play_request->transports
    if(transport_play_request->transports) { 
    ...
    if (transport_play_request->transports) {
    list_ForEach(transportsListEntry, transport_play_request->transports) {
...
transport_play_request_t *transport_play_request_parseFromJSON(cJSON *transport_play_requestJSON){
    ...
    transport_play_request_t *transport_play_request_local_var = NULL;

    // transport_play_request->transports
    cJSON *transports = cJSON_GetObjectItemCaseSensitive(transport_play_requestJSON, "transports");
    ...
    transport_play_request_local_var = transport_play_request_create (
